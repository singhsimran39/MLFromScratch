...
index.ofInformativeAttributes <- attributeSelection(total.X, total.Y, 8)

#Predict labels
if (length(index.ofInformativeAttributes) == 1) {
  predicted.labels <- quadraticDA.pOne(index.ofInformativeAttributes, train.X, train.Y, test.X)
} else {
  predicted.labels <- quadraticDA.pGeneral(index.ofInformativeAttributes, train.X, train.Y, test.X)
}
